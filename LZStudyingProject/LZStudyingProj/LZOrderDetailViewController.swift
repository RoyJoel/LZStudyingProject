//
//  LZOrderDetailViewController.swift
//  LZStudyingProject
//
...
class LZOrderDetailViewController: UIViewController {
    var order = Order()
    var completionHandler: ((Order) -> Void)?
    var deleteCompletionHandler: (() -> Void)?
    lazy var addressView: LZAddressCell = {
        let view = LZAddressCell()
        return view
    }()
...
//        paymentNameLabel.text = order.payment.displayName
        priceNumLabel.text = "\(LZDataConvert.getTotalPrice(order.bills))积分"
        priceLabel.text = "总计"
    ...
    @objc func cancelOrder() {
        LZOrderRequest.deleteOrder(id: order.id) { _ in
    ...
    @objc func buy() {
        let vc = LZBillingViewController()
        vc.order = order
        navigationController?.present(vc, animated: true)
    }

    @objc func confirmDelivery() {
        let orderRequest = OrderRequest(id: order.id, bills: order.bills, shippingAddress: order.shippingAddress, payment: order.payment, playerId: LZUser.user.id, createdTime: order.createdTime, payedTime: order.payedTime, state: .Done)
        LZOrderRequest.updateOrder(order: orderRequest) { _ in
            let toastView = UILabel()
    ...
    @objc func changeAddress() {
        ...
        vc.selectedCompletionHandler = { address in
            ...
            let orderRequest = OrderRequest(id: self.order.id, bills: self.order.bills, shippingAddress: self.order.shippingAddress, payment: self.order.payment, playerId: LZUser.user.id, createdTime: self.order.createdTime, payedTime: self.order.payedTime, state: .Done)
            LZOrderRequest.updateOrder(order: orderRequest) { _ in
                let toastView = UILabel()
    ...
    @objc func sentOrder() {
        let vc = LZDeliveryViewController()
        navigationController?.present(vc, animated: true)
    }
}
