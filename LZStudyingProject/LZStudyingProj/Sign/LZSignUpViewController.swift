...
class LZSignUpViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextFieldDelegate {
    ...
    lazy var nextConfigBtn: TMBasicButton = {
        let btn = TMBasicButton()
        return btn
    }()

    lazy var lastConfigBtn: TMBasicButton = {
        let btn = TMBasicButton()
        return btn
    }()
    ...
    override func viewDidLoad() {
        ...
        ageTextField.textField.delegate = self
    ...
    func setUserInfo() {
        accountTextField.textField.text = LZUser.shared.user.loginName
        passwordTextField.textField.text = LZUser.shared.user.password
        nameTextField.textField.text = LZUser.shared.user.name
        iconImageView.image = UIImage(data: LZUser.shared.user.icon.toPng())
        sexTextField.isLeft = LZUser.shared.user.sex == .Man ? true : false
        ageTextField.textField.text = "\(LZUser.shared.user.age)"
    }

    func getUserInfo() {
        LZUser.shared.user.loginName = accountTextField.textField.text ?? ""
        LZUser.shared.user.password = passwordTextField.textField.text ?? ""
        LZUser.shared.user.name = nameTextField.textField.text ?? ""
        LZUser.shared.user.icon = (iconImageView.image?.pngData() ?? Data()).base64EncodedString()
        LZUser.shared.user.sex = sexTextField.isLeft ? .Man : .Woman
        LZUser.shared.user.age = Int(ageTextField.textField.text ?? "0") ?? 0
    }
...
//                        nextConfigBtn.isEnabled = false
                        LZUser.searchPlayer(loginName: text, completionHandler: { res in
                            if res {
                                self.currentIndex -= 1
...
//                                self.nextConfigBtn.isEnabled = true
            ...
            LZUserRequest.signUp { [weak self] token, error in
                ...
                loggedinUser.append(LZUser.shared.user.loginName)
