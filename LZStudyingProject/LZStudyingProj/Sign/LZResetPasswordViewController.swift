//
//  LZResetPasswordViewController.swift
//  LZStudyingProject
//
//  Created by Jason Zhang on 2023/6/5.
//
...
class LZResetPasswordViewController: UIViewController {
    ...
    lazy var reauthBtn: TMBasicButton = {
        let btn = TMBasicButton()
        return btn
    }()

    lazy var submitBtn: TMBasicButton = {
        let btn = TMBasicButton()
        return btn
    }()
    ...
    @objc func submitPassword() {
        let loggedinUsers = (UserDefaults.standard.array(forKey: LZUDKeys.LoggedinUser.rawValue) as? [String] ?? [])
        if let account = accountTextField.textField.text {
            if loggedinUsers.contains(where: { loginName in
                ...
                if let password = resetTextField.textField.text, password == confirmTextField.textField.text {
                    LZUser.shared.user.loginName = account
                    LZUser.shared.user.password = password
                    LZUserRequest.resetPassword { _ in
                    }
                } else {
