//
//  LZConfigSelectionViewController.swift
//  LZStudyingProject
//
...
class LZConfigSelectionViewController: UIViewController {
    ...
    lazy var shoppingCollectionView: TMSelectionCollectionView = {
        let view = TMSelectionCollectionView(frame: .zero, collectionViewLayout: LZCagCollectionViewLayout(itemCount: com.options.count), com: com)
        return view
    }()
    ...
    lazy var quantityView: LZQuantitySelectorView = {
        let view = LZQuantitySelectorView()
        return view
    }()
    ...
    @objc func addOrder() {
        ...
        let order = Order(id: 0, bills: [bill], shippingAddress: LZUser.user.defaultAddress, payment: .weChatOnline, createdTime: Date().timeIntervalSince1970, state: .ToPay)
        vc.order = order
        navigationController?.pushViewController(vc, animated: true)
    }

    @objc func addToCart() {
        let newBill = BillRequest(id: 0, comId: com.id, quantity: quantityView.currentQuantity, optionId: com.options[shoppingCollectionView.selectedRow].id, orderId: LZUser.user.cart)
        LZUser.addToCart(bill: newBill) { _ in
