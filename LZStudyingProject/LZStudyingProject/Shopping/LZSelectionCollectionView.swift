...
class TMSelectionCollectionView: UICollectionView, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    init(frame: CGRect, collectionViewLayout layout: UICollectionViewLayout, com: Commodity) {
        ...
        delegate = self
        dataSource = self
        register(LZCagSelectionCell.self, forCellWithReuseIdentifier: "cagSelections")
    }
    ...
    func collectionView(_: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = dequeueReusableCell(withReuseIdentifier: "cagSelections", for: indexPath) as! LZCagSelectionCell
        cell.setupUI()
    ...
    func select(at indexPath: IndexPath) {
        let cell = cellForItem(at: IndexPath(row: selectedRow, section: 0)) as? LZCagSelectionCell
        cell?.isBeenSelected = false
        selectedRow = indexPath.row
        (selectedCompletionHandler ?? { _ in })(com.options[indexPath.row])

        let newSelectedCell = cellForItem(at: indexPath) as? LZCagSelectionCell
