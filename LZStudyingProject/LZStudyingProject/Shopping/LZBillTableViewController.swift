//
//  LZBillTableViewController.swift
//  LZStudyingProject
//
...
class LZBillTableViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    var bills: [Bill] = []
    ...
    override func viewDidLoad() {
        ...
        billTableView.register(LZComBillingCell.self, forCellReuseIdentifier: "billing")
    ...
    func openCartMode() {
        ...
        LZUser.getCartInfo { cart in
            if cart.bills.count == 0 {
                ...
                let totalNum = LZDataConvert.getTotalPrice(cart.bills)
    ...
    func tableView(_: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let deleteAction = UIContextualAction(style: .destructive, title: NSLocalizedString("Delete", comment: "")) { _, _, _ in
            ...
            let billRequest = BillRequest(id: bill.id, comId: bill.com.id, quantity: bill.quantity, optionId: bill.option.id, orderId: LZUser.user.cart)
            LZUser.deleteBillInCart(bill: billRequest) { cart in
                self.bills = cart.bills
                self.billTableView.reloadData()
    ...
    @objc func enterBillingViewController() {
        ...
        let newOrder = Order(id: 0, bills: bills, shippingAddress: LZUser.user.defaultAddress, payment: .weChatOnline, createdTime: Date().timeIntervalSince1970, state: .ToPay)
        vc.order = newOrder
