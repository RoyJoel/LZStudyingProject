//
//  LZComContentViewController.swift
//  LZStudyingProject
//
...
class LZComContentViewController: UIViewController {
    var com: Commodity = Commodity()

    lazy var imagesView: LZComImagesView = {
        let view = LZComImagesView()
        return view
    }()
    ...
    override func viewDidLoad() {
        ...
        if LZDataConvert.getPriceRange(with: com.options).0 == LZDataConvert.getPriceRange(with: com.options).1 {
            priceView.text = "\(LZDataConvert.getPriceRange(with: com.options).0)积分"
        } else {
            priceView.text = "\(LZDataConvert.getPriceRange(with: com.options).0) - \(LZDataConvert.getPriceRange(with: com.options).1)积分"
        }
    ...
    @objc func selectConfig() {
        let vc = LZConfigSelectionViewController()
        vc.com = com
