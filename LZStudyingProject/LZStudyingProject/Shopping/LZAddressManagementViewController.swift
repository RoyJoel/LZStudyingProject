//
//  LZAddressManagementViewController.swift
//  LZStudyingProject
//
...
class LZAddressManagementViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(LZAddressCell.self, forCellReuseIdentifier: "AddressCell")
        LZAddressRequest.getAddressInfos(ids: LZUser.user.addresss) { addresss in
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let deleteAction = UIContextualAction(style: .destructive, title: NSLocalizedString("删除", comment: "")) { _, _, _ in
            ...
            LZAddressRequest.deleteAddress(id: address.id) { _ in
    ...
    override func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let action = UIAlertAction(title: "是", style: .default) { _ in
            if let address = (self.tableView.cellForRow(at: indexPath) as? LZAddressCell)?.address {
                self.addresss[indexPath.row] = address
    ...
    @objc func addAddress() {
        let vc = LZAddressEditingViewController()
        vc.openAddingMode()
